...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 z-50 hidden w-screen px-3 md:block transition-all duration-1000 ${
        scrollPosition < 500
          ? "bg-opacity-0"
          : "bg-zinc-800 text-zinc-100 bg-opacity-100"
      }`}
    >
      <div className="flex items-center justify-between max-w-6xl mx-auto h-14 md:h-16">
        <a
          href="/"
          className={`text-2xl md:text-3xl text-zinc-900 font-bold border-[3px] px-1 whitespace-nowrap ${
            scrollPosition < 500
